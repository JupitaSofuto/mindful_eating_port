...
public class MEOverrides {
    ...
    public static void generateDefaultConfig() {
        ...
        for (Item item : new Item[]{Items.COOKED_MUTTON, Items.COOKED_RABBIT, Items.COOKED_SALMON, Items.COOKED_COD}) {
            ResourceLocation l = Util.parse(item.getDescriptionId());
            saturation.addProperty(l.toString(), Math.round(10.0F * item.getFoodProperties().getSaturationModifier() - item.getFoodProperties().getNutrition()) / 10.0F);
        }
        ...
        for (Item item : new Item[]{Items.MELON_SLICE, Items.SWEET_BERRIES, Items.GLOW_BERRIES, Items.COOKED_MUTTON, Items.COOKED_RABBIT,
            ...
            speedy.addProperty(Util.parse(item.getDescription().toString()).toString(), true);
        }

        for (Item item: new Item[]{Items.BEETROOT_SOUP,
                Items.MUSHROOM_STEW, Items.SUSPICIOUS_STEW, Items.RABBIT_STEW}) {
            ResourceLocation l = Util.parse(item.getDescriptionId());
            stackability.addProperty(l.toString(), 16);
        }
        ResourceLocation l = Util.parse(Items.CAKE.getDescriptionId());
        stackability.addProperty(l.toString(), 64);
